...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	{
		$state_info->file_name = html_entity_decode($state_info->file_name);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function delete_layout($delete_result = true)
	{
		...
		if($delete_result === 'depend')
		{
			$error_message = '<p>'.$this->l('dependent_table').'</p>';
...
set_exception_handler(function($e){
	show_error($e->getMessage() . ' Press back button to go back', 500, 'Uncaught exception');
});
